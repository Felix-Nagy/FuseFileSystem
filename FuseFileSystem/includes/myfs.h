...
public:
    ...
    int getDriveSpace();
    int readBlockFromFile(u_int32_t block, char* buffer);
    int writeBlockToFile(u_int32_t block, char* buffer);
    void initializeStructures();
    bool fileNameExists(const char* fileName);
    int findFreeBlock();
    u_int32_t countFreeBlocks();
    void setBlockFree(u_int32_t block);
    void setBlockUsed(u_int32_t block);

    void deleteFATEntries(File *file);
};
